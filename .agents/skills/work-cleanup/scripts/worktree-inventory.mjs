...
export function inventory({ base, notesDir = resolve(homedir(), ".codex/notes/work-cleanup"), report = false, git = defaultGit } = {}) {
...
export function run(args) { console.log(renderInventory(inventory({ report: args.includes("--report"), base: valueAfter(args, "--base"), notesDir: valueAfter(args, "--notes-dir", resolve(homedir(), ".codex/notes/work-cleanup")) }))); return 0; }
