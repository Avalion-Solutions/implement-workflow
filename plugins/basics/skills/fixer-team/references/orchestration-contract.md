...
Pass `model` and `reasoning_effort` as separate delegation fields. Never create a combined pseudo-ID. Use the closest available pair and record a fallback.

| Role | Model | Effort |
| --- | --- | --- |
| Build orchestrator | `gpt-5.6-sol` | `xhigh` |
| Brainstorm orchestrator / workers | `gpt-5.6-sol` / `gpt-5.6-terra` | `high` / `medium` |
| Blue orchestrator | `gpt-5.6-terra` | `xhigh` |
| Blue specialist | `gpt-5.6-terra` | `medium` risky; `low` bounded |
| Red orchestrator | `gpt-5.6-terra` | `xhigh` |
| Red specialist | `gpt-5.6-terra` | `xhigh` architecture/data; `high` UI/tests |
| Red final validator | `gpt-5.6-sol` | `high` |
| Fixer Planner / Builder / Adversary / Judge | `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-terra` / `gpt-5.6-sol` | `high` / `medium` / `medium` / `medium` |
