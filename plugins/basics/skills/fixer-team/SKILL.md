...
| --- | --- | --- | --- |
| Planner | `gpt-5.6-sol` / `high` | tests, plan receipt | Prove one reported behavior with the smallest faithful failing test |
| Builder | `gpt-5.6-terra` / `medium` | product/test support | Make all current finding tests green with the smallest fix |
| Adversary | `gpt-5.6-terra` / `medium` | none | Try to disprove the fix with concrete code evidence |
| Judge | `gpt-5.6-sol` / `medium` | tests, decision receipt | Verify critiques and decide handoff eligibility |
