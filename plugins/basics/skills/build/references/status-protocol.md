...
The hook command reads the official Codex JSON event from stdin. `SubagentStart` activates the selected/next queued agent; `SubagentStop` records an explicit `blocked` or `failed` final prefix and otherwise completes that agent only. `PreToolUse` activates the selected task. `PostToolUse` records tool success/failure, but passes only the selected task when the successful shell command exactly matches its declared `--verification-command`. No hook may complete a team, approve work, or accept an unrecorded manifest. `Stop` moves leftover active tasks to `waiting` and records a consistency event without changing run approval, readiness, or merge state. Do not add `UserPromptSubmit` approval automation; only `build-handoff.mjs approve` may bind an interpreted approval to exact plan and scope hashes.
